...
export default function NoteItem({
  ...
  const isOpen = dropdownNoteID === note.id;
      ...
      <div className="border-2 flex flex-col border-neutral-200 hover:bg-[#EEFCFC] rounded-lg p-4 bg-white relative overflow-hidden">
        ...
        >
          ...
          >
            ...
            <p>{isFavorite ? "remove from favorite" : "add to favorites"}</p>
          </div>
          <div
            onClick={() => {
              moveToTrash();
        ...
        <div className="flex items-center justify-between mb-0.5">
          ...
          <div className="flex items-center flex-col gap-3">
            <div
              onClick={() => {
                setDropdownNoteID(isOpen ? null : note.id);
              }}
            ...
            <img
              className={`${
                isFavorite ? "top-12 rotate-180" : "-top-12"
              } absolute  w-7.5 inline-block hover:rotate-6 transition-all duration-900`}
              src={img}
              alt=""
            />
          </div>
        ...
        <h3 className="text-neutral-400 mb-2 text-sm">
          {formatDateEn(note.date_created)}
          {/* {note.date_created} */}
        </h3>
        <p className="line-clamp-3 text-md grow shrink-0">{note.content}</p>
